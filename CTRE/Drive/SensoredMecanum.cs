...
namespace CTRE.Phoenix.Drive
{
    public class SensoredMecanum : ISmartDrivetrain
    {
        ...
        public SensoredMecanum(SmartMotorController m1, SmartMotorController m2, SmartMotorController m3, SmartMotorController m4, SmartMotorController.FeedbackDevice feedbackDevice, bool useLeftFrontSensor, bool useRightFrontSensor, float wheelRadius)
        {
        ...
        private void Drive(Styles.Smart mode, float forward, float strafe, float turn)
        {
            ...
            {
                _1.SetControlMode(ControlMode.kVoltage);
                _2.SetControlMode(ControlMode.kVoltage);
                _3.SetControlMode(ControlMode.kVoltage);
                _4.SetControlMode(ControlMode.kVoltage);
            }
            else if (mode == Styles.Smart.PercentOutput)
            {
                _1.SetControlMode(ControlMode.kPercentVbus);
                _2.SetControlMode(ControlMode.kPercentVbus);
                _3.SetControlMode(ControlMode.kPercentVbus);
                _4.SetControlMode(ControlMode.kPercentVbus);
            }
            else if(mode == Styles.Smart.VelocityClosedLoop)
            {
                _1.SetControlMode(ControlMode.kSpeed);
                _2.SetControlMode(ControlMode.kSpeed);
                _3.SetControlMode(ControlMode.kSpeed);
                _4.SetControlMode(ControlMode.kSpeed);
            }
