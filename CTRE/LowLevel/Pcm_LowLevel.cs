...
{
    public class Pcm_LowLevel : Device_LowLevel
    {
        ...
        public Pcm_LowLevel(uint deviceId)
            : base(deviceId, 0x00,0,0,0,0) // todo
        {
            CTRE.Native.CAN.Send(CONTROL_1 | _baseArbId, 0x00, 8, kDefaultPeriodMs);
        }
        public Pcm_LowLevel(uint deviceId, uint periodMs)
            : base(deviceId, 0x00,0,0,0,0)// todo
        {
            CTRE.Native.CAN.Send(CONTROL_1 | _baseArbId, 0x00, 8, periodMs);
        }
        ...
        public int GetSol(int idx, out bool param)
        {
            param = false;
            if (idx < 0)
                return (int)ErrorCode.CAN_INVALID_PARAM;
            if (idx >= kNumSolChannels)
                return (int)ErrorCode.CAN_INVALID_PARAM;
            int retval = CTRE.Native.CAN.Receive(STATUS_1 | _baseArbId, ref _cache, ref _len);
        ...
        public int SetEnableSol(int idx, bool param)
        {
            ...
            if (idx >= kNumSolChannels)
                return (int)ErrorCode.CAN_INVALID_PARAM;
            int retval = CTRE.Native.CAN.GetSendBuffer(CONTROL_1 | _baseArbId, ref _cache);
            if (retval != 0)
                return retval;
            if (param == false)
                _cache &= ~(1ul << (16 + idx));
